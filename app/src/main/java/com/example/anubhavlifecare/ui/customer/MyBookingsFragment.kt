...
class MyBookingsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<TextView>(R.id.tvTitle).text = getString(R.string.menu_my_bookings)

        val phone = CustomerSessionManager.getPhone(requireContext())
        if (phone.isNullOrBlank()) {
            tvEmpty.text = getString(R.string.phone_required)
            return
        }
...
private class BillAdapter(private val items: List<CustomerBill>) :
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        holder.text.text = buildString {
            appendLine("${b.billNo} — ${b.billDate}")
            appendLine("₹${b.netAmount} paid ₹${b.receivedAmount} pending ₹${b.pendingAmount}")
            b.rescheduleNote?.let { appendLine(it) }
        }
    }
